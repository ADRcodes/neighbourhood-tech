...
function selectWithTagFilter(hasTagFilter) {
  if (!hasTagFilter) return EVENT_SELECT;
  return EVENT_SELECT.replace("event_tags:event_tags", "event_tags:event_tags!inner");
}
...
export async function listEvents({ filters = {}, signal } = {}) {
  ...
  const hasTagFilter = Boolean(filters.tagId);
  let query = client
    .from("events")
    .select(selectWithTagFilter(hasTagFilter), { signal })
  ...
  if (filters.tagId) {
    query = query.eq("event_tags.tag_id", filters.tagId);
  }
...
export async function createEvent(payload) {
  const client = requireSupabase();
  const query = client.from("events").insert(payload).select(EVENT_SELECT).single();
  return run(query);
}

export async function updateEvent(id, updates) {
  const client = requireSupabase();
  const query = client.from("events").update(updates).eq("id", id).select(EVENT_SELECT).single();
  return run(query);
}
...
export const listSavedEvents = (userId) =>
  run(
    requireSupabase()
      .from("saved_events")
      .select(
